...
export const restAPI = (href) => {
  ...
  return {
    fetchExamination: (id) => client.get(id),
...
export const CommonAPI = (href) => {
  ...
  return {
    fetchByParams: (params) => client.get(`?${params}`),
    fetch: (id = null) => client.get(id ? `${id}/` : ""),
    create: (p) => client.post(null, p),
    update: (p) => client.patch(`${p.id}/`, p),
    delete: (id) => client.delete(`${id}/`),
  };
...
export const SchemaAPI = () => {
  const api = CommonAPI("schemas");

  return { ...api, fetchByKind: (k) => api.fetchByParams(`kind=${k}`) };
};
...
export const templateAPI = (href) => {
  ...
  return {
    fetchAll: () => client.get(),
    fetchTemplateByName: (name, kind = null) =>
      client.get(`?title=${name}&kind=${kind}`),
    fetch: (id) => client.get(`${id}/`),
    create: (p) => client.post(null, p),
    update: (p) => client.patch(`${p.id}/`, p),
    delete: (id) => client.delete(`${id}/`),
