...
const allergiesSlice = createSlice({
  ...
  extraReducers: {
    [fetchAllergiesByPid.rejected]: (state, action) =>
      console.log(`fetchAllergiesByPid failed: ${JSON.stringify(action)}`),
    ...
    [updateAllergy.fulfilled]: (state, { payload }) => {
      console.log(
        `updateAllergy slice: ${JSON.stringify(payload)} + ${state.patients}`
      );
    ...
    [deleteAllergy.fulfilled]: (state, action) => {
      state.allByPid[action.meta.arg.pid] = state.allByPid[
        action.meta.arg.pid
      ].filter((p) => p.id !== action.meta.arg.id);
    },
  },
});
